...
export default function ComprehensiveDemoPage() {
  ...
  // Customer Split Payment State
  const [numberOfCards, setNumberOfCards] = useState(2);
  const [cardAmounts, setCardAmounts] = useState<number[]>([6000, 4000]);
  const [completedCards, setCompletedCards] = useState<number[]>([]);
  const [failedCards, setFailedCards] = useState<number[]>([]);
  const [timeRemaining, setTimeRemaining] = useState(600); // 10 minutes

  // Merchant Revenue Splitting State
  const [enableMerchantSplits, setEnableMerchantSplits] = useState(true);
  const [merchantSplits] = useState([
    { label: 'Merchant Revenue', amount: 8500, recipient: 'merchant_123' }, // $85
    { label: 'Platform Fee', amount: 1000, recipient: 'platform_123' }, // $10
    { label: 'Processing Fee', amount: 500, recipient: 'processor_123' }, // $5
  ]);
  ...
  const handleSinglePayment = async () => {
    ...
    try {
      // Step 1: Process customer's single card payment
      await new Promise((resolve) => setTimeout(resolve, 2000));

      if (enableMerchantSplits) {
        setPaymentStatus('customer_payment_complete');

        // Step 2: Process merchant revenue splits
        setPaymentStatus('merchant_splits_processing');
  ...
  const handleSplitPayment = async () => {
    ...
    try {
      // Step 1: Process each customer card
      for (let i = 0; i < numberOfCards; i++) {
        ...
        if (success) {
          setCompletedCards((prev) => [...prev, i]);
        } else {
          setFailedCards((prev) => [...prev, i]);
      ...
      // All customer payments succeeded
      setPaymentStatus('customer_payment_complete');
        ...
        <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
          ...
          <div className="bg-white rounded-lg shadow-md p-6 mb-8">
            <h2 className="text-2xl font-bold text-gray-900 mb-4">
              Customer Payment + Merchant Revenue Splitting
            </h2>
            <p className="text-gray-600 mb-4">
              This demo shows the complete VSplit functionality: customers can
              choose how to pay (single card or split across multiple cards),
              and merchants can optionally split the received revenue between
              multiple recipients.
            </p>
            <div className="grid md:grid-cols-2 gap-6">
              <div className="bg-blue-50 border border-blue-200 rounded-lg p-4">
                ...
                <ul className="text-blue-800 text-sm space-y-1">
                  <li>
                    • <strong>Single Payment:</strong> Pay full amount with one
                    card
                  </li>
                  <li>
                    • <strong>Split Payment:</strong> Divide amount across
                    multiple cards
                  </li>
                  <li>• Automatic refunds if split payments incomplete</li>
                  <li>• Timeout protection for customer security</li>
                </ul>
              </div>
              <div className="bg-green-50 border border-green-200 rounded-lg p-4">
                <h3 className="font-semibold text-green-900 mb-2 flex items-center">
                  <DollarSign className="h-5 w-5 mr-2" />
                  Merchant Revenue Splits:
                </h3>
                <ul className="text-green-800 text-sm space-y-1">
                  <li>
                    • <strong>Merchant:</strong>{' '}
                    {formatAmount(merchantSplits[0].amount)} (
                    {((merchantSplits[0].amount / totalAmount) * 100).toFixed(
                      0
                    )}
                    %)
                  </li>
                  <li>
                    ...
                    {formatAmount(merchantSplits[1].amount)} (
                    {((merchantSplits[1].amount / totalAmount) * 100).toFixed(
                      0
                    )}
                    %)
                  </li>
                  <li>
                    ...
                    {formatAmount(merchantSplits[2].amount)} (
                    {((merchantSplits[2].amount / totalAmount) * 100).toFixed(
                      0
                    )}
                    %)
                  </li>
          ...
          {!paymentType && paymentStatus === 'idle' && (
            ...
            <div className="bg-white rounded-lg shadow-md p-8 mb-8">
              ...
              {/* Merchant Split Toggle */}
              <div className="mt-8 p-4 bg-gray-50 rounded-lg">
                <div className="flex items-center justify-between">
                  <div>
                    <h4 className="font-semibold text-gray-900">
                      Merchant Revenue Splitting
                    </h4>
                    <p className="text-sm text-gray-600">
                      Automatically split revenue after customer payment
                    </p>
                  </div>
                  <label className="relative inline-flex items-center cursor-pointer">
                    <input
                      type="checkbox"
                      checked={enableMerchantSplits}
                      onChange={(e) =>
                        setEnableMerchantSplits(e.target.checked)
                      }
          ...
          {paymentStatus !== 'idle' && (
            <div className="space-y-6">
              ...
              <div className="bg-white rounded-lg shadow-md p-6">
                ...
                {paymentStatus === 'customer_payment_complete' && (
                  <div className="flex items-center text-green-600">
              ...
              {/* Merchant Revenue Splitting Status */}
              {enableMerchantSplits &&
                [
                  'customer_payment_complete',
                  'merchant_splits_processing',
                  'success',
                  ...
                  <div className="bg-white rounded-lg shadow-md p-6">
                    ...
                    {paymentStatus === 'merchant_splits_processing' && (
                      <div className="flex items-center text-blue-600 mb-4">
                        <Clock className="h-5 w-5 mr-2 animate-spin" />
                        Processing revenue splits...
                      </div>
                    )}

                    <div className="space-y-3">
                      {merchantSplits.map((split, index) => (
                        <div
                          key={index}
                          className="flex items-center justify-between p-3 bg-gray-50 rounded"
                        >
                          <div className="flex items-center">
                            {split.label === 'Merchant Revenue' && (
                              <Building className="h-5 w-5 mr-2 text-green-600" />
                            )}
                            {split.label === 'Platform Fee' && (
                              <Users className="h-5 w-5 mr-2 text-blue-600" />
                            )}
                            {split.label === 'Processing Fee' && (
              ...
              {paymentStatus === 'success' && (
                <div className="bg-green-50 border border-green-200 rounded-lg p-6 text-center">
                  ...
                  <p className="text-green-700 mb-4">
                    Customer payment of {formatAmount(totalAmount)} processed
                    {enableMerchantSplits && ' and revenue splits transferred'}
                  </p>
                  <button
                    onClick={resetDemo}
